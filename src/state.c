...
transition_from_listening(struct packet *current_packet,
  ...
  if (current_packet->tcp->syn) {
    ...
    unsigned char *syn_ack =
        init_syn_ack(current_packet, context,
                     htonl(ntohl(current_packet->tcp->seq) + 1), 1, 0);
...
struct state_transition
transition_from_handshake_initiated(struct packet *current_packet,
...
struct state_transition
transition_from_data_transfer(struct packet *current_packet,
                              struct client_context *context,
                              struct options opts) {
  if (current_packet->tcp->fin) {
    ...
    unsigned char *syn_ack =
        init_syn_ack(current_packet, context,
                     htonl(ntohl(current_packet->tcp->seq) + 1), 0, 0);
    unsigned char *fin =
        init_syn_ack(current_packet, context,
                     htonl(ntohl(current_packet->tcp->seq) + 1), 0, 1);
  ...
  unsigned char *syn_ack = init_syn_ack(
      current_packet, context,
      htonl(current_packet->payload_size + ntohl(current_packet->tcp->seq)), 0,
      0);
  struct response *data = safe_malloc(sizeof(struct response));
  data->data = syn_ack;
  data->next = NULL;
...
void handle_transition(struct state_transition transition,
                       struct client_context *context, struct options opts) {
  int size =
  ...
  while (head) {
    int result =
        sendto(context->connection, head->data, size, 0,
...
enum state handle_packet(enum state current_state,
  ...
  case HANDSHAKE_INITIATED:
    transition =
        transition_from_handshake_initiated(current_packet, context, opts);
    break;
  case DATA_TRANSFER:
