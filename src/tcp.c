...
int parse_tcp_header(unsigned char *buffer, int from) {
  ...
  int acknowledgment_number_start =
      sequence_number_start + sequence_number_size;
  int data_offset_start =
      acknowledgment_number_start + acknowledgment_number_size;
  ...
  unsigned long source_port =
      range_hex_to_decimal(buffer, source_port_start, destination_port_start);
  printf("Source Port: %ld\n", source_port);

  unsigned long destination_port = range_hex_to_decimal(
      buffer, destination_port_start, sequence_number_start);
  printf("Destination Port: %ld\n", destination_port);

  unsigned long sequence_number = range_hex_to_decimal(
      buffer, sequence_number_start, acknowledgment_number_start);
  printf("Sequence Number: %ld\n", sequence_number);

  unsigned long acknowledgment_number = range_hex_to_decimal(
      buffer, acknowledgment_number_start, data_offset_start);
  printf("Acknowledgment Number: %ld\n", acknowledgment_number);

  int data_offset_reserved = buffer[data_offset_start];
  int data_offset =
      ((data_offset_reserved >> 4) & 0x0F) * data_offset_word_size;
  ...
  unsigned long window =
      range_hex_to_decimal(buffer, window_size_start, checksum_start);
  printf("Window Size: %ld\n", window);

  printf("Checksum: ");
  print_range(buffer, checksum_start, urgent_pointer_start);

  unsigned long urgent_pointer =
      range_hex_to_decimal(buffer, urgent_pointer_start, options_start);
...
unsigned short tcp_checksum(const struct iphdr *ip, const struct tcphdr *tcp,
  ...
  unsigned short checksum =
      calculate_checksum((unsigned short *)pseudogram, psize);
  free(pseudogram);
  return checksum;
}
